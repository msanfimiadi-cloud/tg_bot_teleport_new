...
- `INVITE_LINK_TTL_HOURS` — срок действия одноразовой invite-ссылки.
...
`PUBLIC_BASE_URL` (или `WEBHOOK_HOST`) также используется для отслеживаемого перехода
`GET /payments/open/{token}`. Пользователь сразу перенаправляется в YooKassa, а первый переход
фиксируется в истории и отправляется администраторам. Без публичного адреса оплата продолжает
работать по прямой ссылке YooKassa, но факт перехода отследить невозможно.
...
- чек-лист платёжной воронки пользователя: анкета, email, отправка и открытие оплаты,
  подтверждение платежа, выдача ссылки в Telegram-чат;
- уведомления администраторам о каждом основном этапе платёжной воронки;
- массовые и индивидуальные уведомления о подключении или продлении подписки;
...
- Добавлен GitHub Actions CI с Python 3.12, PostgreSQL 16, `ruff`, `mypy`, `pytest` и проверкой цикла миграций.
...
- Истечение или отмена подписки меняет учётный статус, но по бизнес-правилу не удаляет пользователя из закрытого Telegram-чата.
